...
MONGO_URI = "mongodb+srv://coclegend580_db_user:LHyXS0cCEK7hrY5R@cluster0.qv9g8k8.mongodb.net/?retryWrites=true&w=majority"
...
def fetch_live_weather(lat, lon):
  ...
  try:
    ...
    print(
        f"Open-Meteo API failed: {e}. Generating 72-hour safe fallback data."
    )
    dates = pd.date_range(
        end=pd.Timestamp.now().floor("h"), periods=72, freq="h"
    )
...
def run_model(request: FrontendRequest):
  ...
  final_temp = gee_temp if gee_temp != 0.0 else input_df.at[input_df.index[-1], 'tempC']
