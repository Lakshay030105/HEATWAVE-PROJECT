...
exports.simulateHeatwave = async (req, res, next) => {
  try {
    const { wardId, tier, recipientPhone: customRecipientPhone } = req.body;
    ...
    // If Severe or Extreme heat spike, dispatch SMS alert to verified phone number
    let smsResult = null;
    if (tier === 'Severe' || tier === 'Extreme') {
      const targetPhone = normalizePhoneNumber(customRecipientPhone || process.env.MY_PHONE_NUMBER || '+918607405507');
      const advisory = ADVISORY_MESSAGES[tier] || 'High heatwave risk advisory.';
      const alertMessage = `[AAROGYA HEAT ALERT] ${wardExists.name}: ${tier} Risk (${forecastTemp}°C)! ${advisory}`;
      ...
      if (targetPhone) {
        ...
        console.log(`Simulated Heat Spike Alert dispatched for ${wardExists.name} (${tier}) to ${targetPhone}`);
      }
    }
